import { OutStreamInterface, PersonInterface, PhotosInterface } from './리팩토링전';

const previousDateFromNow = (days: number) => new Date(Date.now() - 1000 * 60 * 60 * 24 * days);
...
const listRecentPhotos = (outStream: OutStreamInterface, photos: PhotosInterface[]) => {
  photos
...
const renderPerson = (outStream: OutStreamInterface, person: PersonInterface) => {
